...
Statistics has also some philosophical roots.
...
(2) Compute $$\overline x$$, $$\sigma_{x}$$, $$\sigma_{xy}$$ from scratch (without using any module e.g. numpy, statsmodels). Compare the results with the ones from the numpy library and state your observations.
