...
Problem 2: Find the probability of *both children being girls* (\alpha) given that *at least one children is a girl* (\beta).
...
$$
P(\alpha | \beta) = \frac {P(\alpha,\beta)}{P(\beta)} = \frac {P(\alpha)}{P(\beta)}
$$

$$
P(\alpha | \gamma) = \frac {P(\alpha,\gamma)}{P(\gamma)}
$$
