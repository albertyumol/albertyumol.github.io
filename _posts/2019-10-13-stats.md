...
Take as an example the qualifying entrance exam for **Eskwelabs**. Assume that there are 50 items for the exam with 100 examinees. You have already learned by now how to generate synthetic data in Python. We generate the scores randomly and visualize using a histogram with the hope that no one scores below 50.
