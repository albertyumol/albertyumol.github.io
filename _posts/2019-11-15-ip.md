...
Going back to the previous GANs, most of them uses **batch normalization** to boost speed and training stability. This is done through this equation:

$$
y = \frac{x - \mathrm{E}[x]}{\sqrt{\mathrm{Var}[x] + \epsilon}} * \gamma + \beta
$$

Essentially, the mean of the features are translated to 0 and the standard deviation to 1. However, this method is still gruesome

but, this slows down the training process because each layer must learn to adapt themselves to a new distribution in every training step


used to significantly speed up and stabilize training by having the mean moved to 0 and standard deviation to 1 before a pair of channel parameters and allow them to “denormalize” again


This method performs well except when it doesn't. Case in point
